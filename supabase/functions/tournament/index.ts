...
async function insertRound(admin: SupabaseClient, tid: string, round: number, pairings: Pairing[]) {
  for (const p of pairings) {
    if (!p.black) {
      ...
      await admin
        .from('tournament_players')
        .update({ score: Number((data as { score?: number } | null)?.score ?? 0) + 1 })
        .eq('tournament_id', tid)
        .eq('user_id', p.white);
    } else {
      await admin.from('matches').insert({
...
serve(async (req) => {
  ...
  if (action === 'start') {
    ...
    await admin
      ...
      .eq('id', tournamentId);
    await insertRound(admin, tournamentId, 1, firstRound);
  ...
  if (action === 'advance') {
    ...
    if (tour.format === 'roundrobin') {
      ...
      await insertRound(admin, tournamentId, nextRound, schedule[nextRound - 1].pairings);
    ...
    await admin.from('tournaments').update({ current_round: nextRound }).eq('id', tournamentId);
    await insertRound(admin, tournamentId, nextRound, next.pairings);
    return json({ ok: true, round: nextRound });
  }
