            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <div className="flex gap-1 rounded-full bg-plaster-2 p-1">
                {(['roundrobin', 'knockout'] as const).map((f) => (
                  <button key={f} onClick={() => setFormat(f)} className={`rounded-full px-3 py-1.5 text-sm font-semibold ${format === f ? 'bg-ink text-white' : 'text-ink-soft'}`}>
                    {f === 'roundrobin' ? 'Round-robin' : 'Knockout'}
                  </button>
                ))}
              </div>
              <button onClick={create} disabled={busy || !name.trim()} className="btn-gold">Create</button>
            </div>
          </div>
